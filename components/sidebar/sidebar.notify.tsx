...
const SidebarNotify: FC<Props> = ({ notifies }) => {
    ...
    const onClickToNotifyAction = (noti: INotify) => {
        const actions: {[key: string]: () => void} = {
            "receive_friend": () => console.log("nhan loi moi ket ban"),
            "receive_message": () => console.log("nhan tin nhan")
        }
    ...
    return <PopoverContent className="w-[320px] border-x-2 border-sky-500 p-0 rounded-t-lg">
        ...
        <div className="space-y-2 h-[300px] overflow-y-auto scrollbar-none">
            {notifies?.map(noti => {
                ...
                return <div key={noti._id} onClick={()=>onClickToNotifyAction(noti)} className="group hover:bg-sky-500 px-2 py-1 duration-200 cursor-pointer flex items-center space-x-2">
